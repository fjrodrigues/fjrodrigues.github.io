...
function toggleActive() {
	...
	{
		var elements = Array.from(document.getElementsByClassName(newActiveItem));
		elements.forEach(element => element.classList.toggle("active-item"));
	}
	else
	{
		var elements = Array.from(document.getElementsByClassName(this.activeItem));
		elements.forEach(element => element.classList.remove("active-item"));
		var newElements = Array.from(document.getElementsByClassName(newActiveItem));
		newElements.forEach(element => element.classList.toggle("active-item"));
	}
...
function toggleFictionBlock(target) {
	...
	let timelineBlockElements = document.getElementsByClassName('timeline-block');
	for(let j=0; j<timelineBlockElements.length; j++) {
		if(timelineBlockElements[j].classList.contains(targetClassIdentifier))
		{
...
function scrollToSection(){
	$('html, body').stop(true, false).animate({ scrollTop: $('#video_section').offset().top}, 800);
}
...
function pauseOthers( excludePlayer) {
	let listOfPlayersIds = ["video_pair-1", "video_pair-2", "video_pair-3", "video_pair-4"];
